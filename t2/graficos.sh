...
# Tempo
for funcao in "gera_sl resolve_sl calcula_residuos"; do
...
for funcao in "gera_sl resolve_sl calcula_residuos"; do
    gnuplot -persist <<- EOFMarker
        ...
        plot "$DADOS/operacoes_aritmeticas_dp_$funcao"_v1.dat" title "DP: $funcao - v1" lt 7 lc 7 w lp, "$DADOS/operacoes_aritmeticas_dp_$funcao"_v2.dat" title "DP: $funcao - v2" lt 7 lc 6 w lp, "$DADOS/operacoes_aritmeticas_avx_dp_$funcao"_v1.dat" title "AVX: $funcao - v1" lt 7 lc 2 w lp, "$DADOS/operacoes_aritmeticas_avx_dp$funcao"_v2.dat" title "AVX: $funcao - v2" lt 7 lc 1 w lp
EOFMarker
done
...
i=0
for teste in "banda_de_memoria cache_miss_l2"; do
    for funcao in "gera_sl calcula_residuos"; do
        gnuplot -persist <<- EOFMarker
            set logscale x
